...
final class RequestTests: XCTestCase {
    ...
    /// Every response type on the chain is an answer, and every answer moves the status.
    ///
    /// Conversation is deliberately not on this list — it lives in `requests/{id}/messages` and
    /// cannot reach the status at all. See `CommentTests`.
